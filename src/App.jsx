        ...
        {showIntro ? (
          ...
          <main>
            ...
            <nav className="fixed top-0 w-full z-40 bg-black/90 backdrop-blur-md border-b border-white/10 px-4 md:px-6 py-3 flex flex-wrap justify-between items-center gap-4">
              ...
              {/* ÁREA DIREITA - Botão Cliente + Secure */}
              <div className="flex items-center gap-4">
                
                {/* --- BOTÃO CLIENTE VOLTZ (ANIMADO) --- */}
                <a href="https://gestao-adriano.vercel.app/" target="_blank" rel="noopener noreferrer">
                  <motion.button
                    animate={{ 
                      opacity: [1, 0.6, 1], // Efeito de desmanhecer e reviver
                      scale: [1, 1.02, 1],   // Leve pulsação de tamanho
                      boxShadow: ["0px 0px 0px rgba(255, 215, 0, 0)", "0px 0px 10px rgba(255, 215, 0, 0.5)", "0px 0px 0px rgba(255, 215, 0, 0)"] // Brilho
                    }}
                    transition={{ 
                      duration: 2, 
                      repeat: Infinity, 
                  ...
                  >
                    <Zap size={16} fill="black" /> {/* Ícone de Raio */}
                    Cliente Voltz
                  </motion.button>
                </a>

                {/* Badge Secure (Mantive menor para dar destaque ao Voltz) */}
            ...
            <section className="min-h-screen flex flex-col items-center justify-center px-4 relative pt-20">
              ...
              >
                ...
                >
                  <div className="absolute inset-0 bg-[#FFD700] blur-xl opacity-40 rounded-full"></div> {/* Mudei o brilho para amarelo Voltz */}
                  
                  <img 
                    src="/perfil.jpg" 
                    alt="Adriano Ferreira" 
                    className="relative w-40 h-40 md:w-48 md:h-48 object-cover rounded-full border-4 border-[#FFD700] shadow-2xl"
                  />
                ...
                <p className="text-gray-400 text-lg md:text-xl mb-8">
                  CEO da <span className="text-[#FFD700] font-bold">Voltz Soluções</span>. Transformando ideias em software seguro.
                </p>

                {/* BOTÕES DE AÇÃO */}
                <div className="flex flex-col sm:flex-row gap-4 justify-center w-full">
                  <a href={`https://wa.me/${WHATSAPP}`} target="_blank" className="bg-[#FFD700] hover:bg-yellow-400 text-black px-8 py-3 rounded-lg font-bold flex items-center justify-center gap-2 transition-all hover:shadow-[0_0_20px_rgba(255,215,0,0.5)]">
                    <MessageCircle size={20} /> Orçamento Voltz
                  </a>
                  
                  <a href={GITHUB_URL} target="_blank" className="bg-neutral-800 hover:bg-neutral-700 text-white px-8 py-3 rounded-lg font-bold flex items-center justify-center gap-2 border border-white/10">
            ...
            <section className="py-24 px-4 bg-black/50 border-t border-white/10">
              <div className="max-w-6xl mx-auto">
                <h2 className="text-3xl font-bold mb-12 flex items-center gap-2">
                  <Code2 className="text-[#FFD700]"/> Projetos Recentes
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {projects.map((project, index) => (
                    <motion.div
                      ...
                      className="group bg-neutral-900 border border-white/10 rounded-xl p-6 hover:border-[#FFD700]/50 hover:bg-neutral-800 transition-all"
                    >
                      ...
                      <a href={project.link} target="_blank" className="inline-flex items-center gap-2 text-sm font-bold text-white hover:text-[#FFD700]">
